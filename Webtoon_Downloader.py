...
def getRootHtmlDaemon(op, webtoonId, cookie):
    global rootHtml
    if 'rootHtml' in globals():
        return rootHtml
    while True:
        td = getRootHtml(op, webtoonId, cookie)
        if td == -1:
            time.sleep(0.5)
            continue
        rootHtml = td
            ...
            if not qu.empty() and runningThreadNo.value < multiThreadMergingCount:
                ...
                if mergeOption == 1:
                    thr = Process(target=mergeImage,
                                  args=(
                                  op, webtoonId, targetEpisode, imgNo[targetEpisode], saveDir, runningThreadNo, cookie))
                if mergeOption == 2:
...
if __name__ == '__main__':
    ...
    if len(sys.argv) - 1 > 8:
        ...
        if sys.argv[1] == 'kakao':
            cookie = KCookie(sys.argv[9], sys.argv[10], sys.argv[11], sys.argv[12], sys.argv[13])
    else:
        cookie = None
